...
public:
  ...
  std::vector<Detection> detect(const cv::Mat &frame, float confThres = 0.25f) {
    ...
    const std::vector<float> inputTensorValues =
        preprocess(frame, ratio, dw, dh);

    // 构造 ONNX 输入张量 [1,3,H,W]
    std::array<int64_t, 4> tensorShape = {1, 3, inputH_, inputW_};
    Ort::Value inputTensor = Ort::Value::CreateTensor<float>(
        memoryInfo_, const_cast<float *>(inputTensorValues.data()),
        inputTensorValues.size(), tensorShape.data(), tensorShape.size());
    ...
    std::vector<Ort::Value> outputs =
        session_.Run(Ort::RunOptions{nullptr}, inputNames.data(), &inputTensor,
